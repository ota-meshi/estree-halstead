...
const EXTRACT_TOKENS: AllVisitor = {
  ...
  AwaitExpression(this: ExtractTokensContext, node: TSESTree.AwaitExpression) {
  ...
  ClassExpression(this: ExtractTokensContext, node: TSESTree.ClassExpression) {
    this.operators.add("class");
    if (node.superClass) {
      this.operators.add("extends");
    }
  ...
  ExportSpecifier(this: ExtractTokensContext, node: TSESTree.ExportSpecifier) {
    if (node.exportKind === "type") {
      this.operators.add("type");
    }
  ...
  ImportSpecifier(this: ExtractTokensContext, node: TSESTree.ImportSpecifier) {
    if (node.importKind === "type") {
      this.operators.add("type");
    }
  ...
  UnaryExpression(this: ExtractTokensContext, node: TSESTree.UnaryExpression) {
    this.operators.add(node.operator);
  ...
  YieldExpression(this: ExtractTokensContext, node: TSESTree.YieldExpression) {
    this.operators.add("yield");
    if (node.delegate) {
      this.operators.add("*");
    }
    if (node.argument && getPrecedence(node) > getPrecedence(node.argument)) {
      this.operators.add("()");
    }
  },
